...
#define FLAG_REMOVE_VALVE_HAT		(1 << 6)
...
int TFClassType:g_iPlayerSpawnClass[MAXPLAYERS + 1] = { TFClass_Unknown, ... };
...
Handle g_hCvarAdminOnly			= INVALID_HANDLE;
Handle g_hCvarAdminFlags		   = INVALID_HANDLE;
Handle g_hCvarAdminOverride		= INVALID_HANDLE;
Handle g_hCvarAnnounce			 = INVALID_HANDLE;
Handle g_hCvarAnnouncePlugin	   = INVALID_HANDLE;
Handle g_hCvarForceDefaultOnUsers  = INVALID_HANDLE;
Handle g_hCvarForceDefaultOnAdmins = INVALID_HANDLE;
Handle g_hCvarDelayOnSpawn		 = INVALID_HANDLE;
Handle g_hCvarBlockTriggers		= INVALID_HANDLE;
Handle g_hCvarFileList			= INVALID_HANDLE;

// ---- Others ---------------------------------------------------------------------
Handle g_hCookies[TFClassType] = { INVALID_HANDLE, ... };
...
//Handle g_hMenuEquip  = INVALID_HANDLE;
//Handle g_hMenuRemove = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	// Create plugin cvars
	g_hCvarVersion			  = CreateConVar("tf_models_version", PLUGIN_VERSION, PLUGIN_NAME, CVAR_FLAGS|FCVAR_SPONLY|FCVAR_REPLICATED);
	...
	g_hCvarAdminOverride		= CreateConVar("tf_models_admin_override",  "0",   "Administrators can override the model restrictions.",		   CVAR_FLAGS);
	...
	g_hCvarBlockTriggers		= CreateConVar("tf_models_blocktriggers",   "1",   "Blocks the triggers so they won't spam on the chat.",			   CVAR_FLAGS);
	g_hCvarFileList				= CreateConVar("tf_models_list",	"TF2_ItemList",	"Which file to use as the hat list, relative to sourcemod/configs folder",	CVAR_FLAGS);

	// Create cookies
	g_hCookies[TFClass_DemoMan] =	RegClientCookie("tf_models_demoman_0", "", CookieAccess_Public);
//	g_hCookies[TFClass_DemoMan][1]  = RegClientCookie("tf_models_demoman_1", "", CookieAccess_Public);
//	g_hCookies[TFClass_DemoMan][2]  = RegClientCookie("tf_models_demoman_2", "", CookieAccess_Public);

	g_hCookies[TFClass_Engineer] = RegClientCookie("tf_models_engineer_0", "", CookieAccess_Public);
//	g_hCookies[TFClass_Engineer][1] = RegClientCookie("tf_models_engineer_1", "", CookieAccess_Public);
//	g_hCookies[TFClass_Engineer][2] = RegClientCookie("tf_models_engineer_2", "", CookieAccess_Public);

	g_hCookies[TFClass_Heavy]	= RegClientCookie("tf_models_heavy_0", "", CookieAccess_Public);
//	g_hCookies[TFClass_Heavy][1]	= RegClientCookie("tf_models_heavy_1", "", CookieAccess_Public);
//	g_hCookies[TFClass_Heavy][2]	= RegClientCookie("tf_models_heavy_2", "", CookieAccess_Public);

	g_hCookies[TFClass_Medic]	= RegClientCookie("tf_models_medic_0", "", CookieAccess_Public);
//	g_hCookies[TFClass_Medic][1]	= RegClientCookie("tf_models_medic_1", "", CookieAccess_Public);

	g_hCookies[TFClass_Pyro]	 = RegClientCookie("tf_models_pyro_0", "", CookieAccess_Public);
//	g_hCookies[TFClass_Pyro][1]	 = RegClientCookie("tf_models_pyro_1", "", CookieAccess_Public);

	g_hCookies[TFClass_Scout]	= RegClientCookie("tf_models_scout_0", "", CookieAccess_Public);
//	g_hCookies[TFClass_Scout][1]	= RegClientCookie("tf_models_scout_1", "", CookieAccess_Public);

	g_hCookies[TFClass_Sniper]   = RegClientCookie("tf_models_sniper_0", "", CookieAccess_Public);
//	g_hCookies[TFClass_Sniper][1]   = RegClientCookie("tf_models_sniper_1", "", CookieAccess_Public);

	g_hCookies[TFClass_Soldier]  = RegClientCookie("tf_models_soldier_0", "", CookieAccess_Public);
//	g_hCookies[TFClass_Soldier][1]  = RegClientCookie("tf_models_soldier_1", "", CookieAccess_Public);

	g_hCookies[TFClass_Spy]	  = RegClientCookie("tf_models_spy_0", "", CookieAccess_Public);
//	g_hCookies[TFClass_Spy][1]	  = RegClientCookie("tf_models_spy_1", "", CookieAccess_Public);
...
Item_Equip(client, iItem)
{
	...
	if (IsPlayerAlive(client))
	{
		...
		if (!(g_iItemFlags[iItem] & FLAG_INVISIBLE))
		{
			...
			AcceptEntityInput(client, "SetCustomModel");
			if (g_iItemFlags[iItem] & FLAG_NO_ANIM)
			{
				SetEntProp(client, Prop_Send, "m_bCustomModelRotates", 1);
		...
		if (g_iItemFlags[iItem] & FLAG_REMOVE_VALVE_HAT) RemoveValveHat(client);
	}
}
...
//		KvGetString(kvItemList, "admin", strLine, sizeof(strLine)); g_iItemAdmin[g_iItemCount] = String
...
//		if (g_iItemFlags[g_iItemCount] & FLAG_INVISREQ) g_Invis_Req[g_iItemCount] = 1;
		...
		if (!(g_iItemFlags[g_iItemCount] & FLAG_INVISIBLE))
		{
			...
			// Check if the admin wants to use a Valve model, warn them that it breaks hats.
...
Item_ParseFlags(String:strFlags[])
{
	...
	if (StrContains(strFlags, "REMOVE_VALVE", false)			!= -1)	Flags |= FLAG_REMOVE_VALVE_HAT;
...
Item_IsWearable(client, Item)
{
	...
	if (g_iPlayerFlags[client] & PLAYER_OVERRIDE) return 1;

	if (g_iPlayerFlags[client] & PLAYER_ADMIN)
	{
...
Item_IsWearable_Admin_Force(client,Item)
{
	// If the selected item is not valid, it can't be wearable! Rargh!
	if (Item < 0 || Item >= g_iItemCount) return 0;
