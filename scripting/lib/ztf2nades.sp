...
public OnPluginStart()
{
    ...
    cvHelpLink = CreateConVar("sm_tf2nades_helplink", "", "web page with info on the TF2NADES plugin");
...
GetNade(client)
{
    ...
    if (makenade)
    {
        ...
        if (nade > 0 && IsValidEntity(nade))
        {
            SetEntPropEnt(nade, Prop_Send, "m_hOwnerEntity", client);
                ...
                if (GameType == tf2)
                {
                    ...
                    for (new i=MaxClients+1; i<GetMaxEntities(); i++)
                    {
                        if (IsValidEdict(i) && IsValidEntity(i))
                        {
                            ...
                            {
                                if (GetEntProp(i, Prop_Send, "m_iTeamNum") == oteam)
                                {
    ...
    for (new i=MaxClients+1; i<count; i++)
    {
        if (IsValidEdict(i) && IsValidEntity(i))
        {
            ...
            if (StrEqual(tName, "obj_sentrygun")
                ...
                || StrEqual(tName, "obj_teleporter") )
            {
                objteam=GetEntProp(i, Prop_Send, "m_iTeamNum");
                if (team!=objteam)
                {
