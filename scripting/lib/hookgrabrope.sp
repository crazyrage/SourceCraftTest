...
public OnPluginStart()
{
    ...
    // Find offsets
    FindSendPropInfo("CBasePlayer", "m_vecVelocity[0]", .local_offset=GetVelocityOffset_x);
    if(GetVelocityOffset_x == -1)
        SetFailState("[HGR] Error: Failed to find the velocity_x offset, aborting");

    FindSendPropInfo("CBasePlayer", "m_vecVelocity[1]", .local_offset=GetVelocityOffset_y);
    if(GetVelocityOffset_y == -1)
        SetFailState("[HGR] Error: Failed to find the velocity_y offset, aborting");

    FindSendPropInfo("CBasePlayer", "m_vecVelocity[2]", .local_offset=GetVelocityOffset_z);
...
{
    if (ClientAccess(client, Give, action))
    {
        if (duration > 0.0)
        {
...
{ 
    ...
    ClientAccess(client, Take, action);
    return Plugin_Stop;
}
