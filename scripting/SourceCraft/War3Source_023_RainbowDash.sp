...
{    
    if(num==230)
    {
        ...
        thisRaceID=CreateRace("rainbowdash", .name="[MLP:FIM] Rainbow Dash", .faction=Pony, .type=Biological, .required_level=16);
#else
        ...
        SetUpgradeCost(thisRaceID, SKILL_EVADE, 10);
        SetUpgradeCost(thisRaceID, SKILL_SWIFT, 10);

        SetUpgradeCost(thisRaceID, SKILL_SPEED, 30);
        SetUpgradeCategory(thisRaceID, SKILL_SPEED, 2);
        SetUpgradeCooldown(thisRaceID, SKILL_SPEED, 20.0);
        SetUpgradeEnergy(thisRaceID, SKILL_SPEED, GetUpgradeCooldown(thisRaceID,SKILL_SPEED));

        SetUpgradeCost(thisRaceID, ULTIMATE, 30);
        SetUpgradeCategory(thisRaceID, ULTIMATE, 1);
        SetUpgradeCooldown(thisRaceID, ULTIMATE, 20.0);
        SetUpgradeEnergy(thisRaceID, ULTIMATE, GetUpgradeCooldown(thisRaceID,ULTIMATE));

        // Get Configuration Data

        GetConfigFloatArray("chance",  fEvadeChance, sizeof(fEvadeChance),
                fEvadeChance, thisRaceID, SKILL_EVADE);

        GetConfigFloatArray("attack_speed",  fSwiftASPDBuff, sizeof(fSwiftASPDBuff),
                fSwiftASPDBuff, thisRaceID, SKILL_SWIFT);

        GetConfigFloatArray("speed",  abilityspeed, sizeof(abilityspeed),
                abilityspeed, thisRaceID, SKILL_SPEED);

        GetConfigFloatArray("radius",  rainboomradius, sizeof(rainboomradius),
                rainboomradius, thisRaceID, ULTIMATE);
#endif
