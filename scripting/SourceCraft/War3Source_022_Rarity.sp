...
public void OnWar3LoadRaceOrItemOrdered(num)
{
    if(num==220)
    {
        ...
        thisRaceID=CreateRace("rarity", .faction=Pony, .type=Biological, .required_level=48);
#else
        ...
        SetUpgradeCost(thisRaceID, SKILL_HEARTACHE, 20);
        SetUpgradeEnergy(thisRaceID, SKILL_HEARTACHE, 1.0);

        SetUpgradeCost(thisRaceID, SKILL_SMITTEN, 5);
        ...
        SetUpgradeCost(thisRaceID, SKILL_SLEEP, 30);
        SetUpgradeCategory(thisRaceID, SKILL_SLEEP, 2);
