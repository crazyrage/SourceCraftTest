...
public void OnWar3PluginReady()
{
    ...
    ULT=War3_AddRaceSkill(thisRaceID,"Death Pact","Gain Health from killing targets",true,4); 
        ...
        SetUpgradeCost(thisRaceID, SKILL_1, 20);
        SetUpgradeEnergy(thisRaceID, SKILL_1, 1.0);

        SetUpgradeCost(thisRaceID, SKILL_2, 20);
        SetUpgradeEnergy(thisRaceID, SKILL_2, 1.0);

        SetUpgradeCost(thisRaceID, ULT, 30);
        SetUpgradeCategory(thisRaceID, ULT, 1);
        SetUpgradeEnergy(thisRaceID, ULT, 10.0);

        SetUpgradeCost(thisRaceID, SKILL_3, 30);
        SetUpgradeCategory(thisRaceID, SKILL_3, 2);
        SetUpgradeCooldown(thisRaceID, SKILL_3, 18.0);
        SetUpgradeEnergy(thisRaceID, SKILL_3, GetUpgradeCooldown(thisRaceID,SKILL_3));

        // Get Configuration Data
        GetConfigArray("num_arrows",  StrafeArrows, sizeof(StrafeArrows),
                StrafeArrows, thisRaceID, SKILL_1);

        SearingArrowChance=GetConfigFloat("chance", SearingArrowChance, thisRaceID, SKILL_2);

        ultCircleEnable=bool:GetConfigNum("circle", ultCircleEnable, thisRaceID, ULT);

        GetConfigArray("time",  WindWalkTime, sizeof(WindWalkTime),
                WindWalkTime, thisRaceID, SKILL_3);

        GetConfigArray("invisibility",  WindWalkInvis, sizeof(WindWalkInvis),
                WindWalkInvis, thisRaceID, SKILL_3);

        GetConfigArray("regain-from",  regain0, sizeof(regain0),
                regain0, thisRaceID, ULT);

        GetConfigArray("regain-to",  regain1, sizeof(regain1),
                regain1, thisRaceID, ULT);
#endif
...
public void OnW3TakeDmgBullet(victim,attacker,Float:damage)//OnWar3EventPostHurt( victim, attacker, damage )
{
    if( W3GetDamageIsBullet() && ValidPlayer( victim, true ) && ValidPlayer( attacker, true ) && GetClientTeam( victim ) != GetClientTeam( attacker ) )
    {
        if( War3_GetRace( attacker ) == thisRaceID && !W3HasImmunity(victim,Immunity_Skills) && !Hexed( attacker, false ))
        {
            ...
            {
#if defined SOURCECRAFT
                if( GetRandomFloat( 0.0, 1.0 ) <= SearingArrowChance && CanInvokeUpgrade(attacker,thisRaceID,SKILL_2, .notify=false))
#else
...
public void OnWar3EventDeath(victim, attacker, deathrace)
{
    ...
    if(race==thisRaceID && skill>0 && ValidPlayer( victim, false ) && ValidPlayer( attacker, true ) && GetClientTeam( victim ) != GetClientTeam( attacker ) )
    {
        ...
        if (CanInvokeUpgrade(attacker,thisRaceID,ULT, .notify=false))
        {
            ...
            {
                hpadd = RoundToNearest(energy + used);
                SetEnergy(attacker, 0.0);
            }
#endif
...
public void OnWar3EventSpawn(client)
{
    if(War3_GetRace(client)==thisRaceID)
    {
        ...
        War3_SetBuff(client,fInvisibilitySkill,thisRaceID,1.0);
        War3_SetBuff(client,fMaxSpeed,thisRaceID,1.0);
        CreateTesla(client,1.0,3.0,10.0,60.0,3.0,4.0,600.0,"160","200","255 25 25","ambient/atmosphere/city_skypass1.wav","sprites/tp_beam001.vmt",true);
    }
}
