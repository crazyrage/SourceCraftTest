...
#else
    LoadTranslations("w3s.race.lich.phrases");
#endif
}
...
public void OnWar3LoadRaceOrItemOrdered(num)
{
    ...
    {
#if defined SOURCECRAFT
        thisRaceID=CreateRace("lich", .faction=UndeadScourge, .type=Undead, .required_level=16);
#else
        ...
        SetUpgradeCost(thisRaceID, SKILL_FROSTARMOR, 5);

        SetUpgradeCost(thisRaceID, SKILL_DARKRITUAL, 10);
        SetUpgradeEnergy(thisRaceID, SKILL_DARKRITUAL, 1.0);

        SetUpgradeCost(thisRaceID, SKILL_FROSTNOVA, 30);
        SetUpgradeCategory(thisRaceID, SKILL_FROSTNOVA, 2);
        SetUpgradeCooldown(thisRaceID, SKILL_FROSTNOVA, AbilityCooldownTime);
        AbilityCooldownTime = GetUpgradeCooldown(thisRaceID,SKILL_FROSTNOVA);
        SetUpgradeEnergy(thisRaceID, SKILL_FROSTNOVA, AbilityCooldownTime);

        SetUpgradeCost(thisRaceID, ULT_DEATHDECAY, 30);
        SetUpgradeCategory(thisRaceID, ULT_DEATHDECAY, 1);
        SetUpgradeCooldown(thisRaceID, ULT_DEATHDECAY, 30.0);
        SetUpgradeEnergy(thisRaceID, ULT_DEATHDECAY, GetUpgradeCooldown(thisRaceID,ULT_DEATHDECAY));

        // Get Configuration Data
        FrostNovaRadius=GetConfigFloat("radius", FrostNovaRadius, thisRaceID, SKILL_FROSTNOVA);

        GetConfigFloatArray("speed",  FrostNovaArr, sizeof(FrostNovaArr),
                FrostNovaArr, thisRaceID, SKILL_FROSTNOVA);

        GetConfigFloatArray("armor",  FrostArmorAmount, sizeof(FrostArmorAmount),
                FrostArmorAmount, thisRaceID, SKILL_FROSTARMOR);

        GetConfigArray("hp",  DarkRitualAmt, sizeof(DarkRitualAmt),
                DarkRitualAmt, thisRaceID, SKILL_DARKRITUAL);

        ultmaxdistance=GetConfigFloat("range", ultmaxdistance, thisRaceID, ULT_DEATHDECAY);

        GetConfigArray("amount",  DeathDecayAmt, sizeof(DeathDecayAmt),
                DeathDecayAmt, thisRaceID, ULT_DEATHDECAY);
#endif
...
public void OnW3TakeDmgBullet(victim,attacker,Float:damage)
{
    ...
    if(War3_GetRace(victim)==thisRaceID&&ValidPlayer(attacker,true))
    {
        if(GetClientTeam(victim)!=GetClientTeam(attacker))
        {
            ...
            int skill_frostarmor =War3_GetSkillLevel(victim,thisRaceID,SKILL_FROSTARMOR);
...
public void OnWar3EventDeath(victim, attacker, deathrace)
{
    ...
    for(int i =1;i<=MaxClients;i++)
    {
        if(War3_GetRace(i)==thisRaceID)
        {
            ...
            {
                int skill =War3_GetSkillLevel(i,thisRaceID,SKILL_DARKRITUAL);
                if(skill>0 && !Silenced(i))
                {
#if defined SOURCECRAFT
                    if (CanInvokeUpgrade(i,thisRaceID,SKILL_DARKRITUAL, .notify=false))
                    {
#endif
                    int hpadd =DarkRitualAmt[skill];
        ...
        if(ult_level>0)
        {
            if(War3_SkillNotInCooldown(client,thisRaceID,ULT_DEATHDECAY,true))
            {
                if(!Silenced(client))
                {
                    ...
                    {
                        int damage =DeathDecayAmt[ult_level];
                        int damagedealt;
                        for(int i =0;i<targetsfound;i++)
                        {
                            int victim =targetlist[i];
