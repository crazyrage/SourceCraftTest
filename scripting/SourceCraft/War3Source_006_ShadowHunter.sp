...
int SKILL_HEALINGWAVE, SKILL_HEX, SKILL_WARD, ULT_VOODOO;

//skill 1
float HealingWaveAmountArr[]={0.0,1.0,2.0,3.0,4.0};
float HealingWaveDistance=500.0;
int ParticleEffect[MAXPLAYERSCUSTOM][MAXPLAYERSCUSTOM]; // ParticleEffect[Source][Destination]

//skill 2
float HexChanceArr[]={0.00,0.025,0.05,0.075,0.100};

//skill 3
int MaximumWards[]={0,1,2,3,4}; 
int WardDamage[]={0,1,2,3,4};
...
//char ultimateSound[]="war3source/divineshield.wav";
//char wardDamageSound[]="war3source/thunder_clap.wav";
...
#if defined SOURCECRAFT
    LoadTranslations("w3s.race.shadow_hunter.phrases");
#else
...
public OnWar3LoadRaceOrItemOrdered(num)
{
    ...
    {
#if defined SOURCECRAFT
        thisRaceID=CreateRace("shadow_hunter", .faction=OrcishHorde, .type=Biological, .required_level=48);
#else
        ...
        SetUpgradeCost(thisRaceID, SKILL_HEALINGWAVE, 10);

        SetUpgradeCost(thisRaceID, SKILL_HEX, 0);
        SetUpgradeRecurringEnergy(thisRaceID, SKILL_HEX, 2.0);

        SetUpgradeCost(thisRaceID, SKILL_WARD, 30);
        SetUpgradeCategory(thisRaceID, SKILL_WARD, 2);
        SetUpgradeEnergy(thisRaceID, SKILL_WARD, 5.0);

        SetUpgradeCost(thisRaceID, ULT_VOODOO, 30);
        SetUpgradeCategory(thisRaceID, ULT_VOODOO, 1);
        SetUpgradeCooldown(thisRaceID, ULT_VOODOO, 20.0);
        SetUpgradeEnergy(thisRaceID, ULT_VOODOO, GetUpgradeCooldown(thisRaceID,ULT_VOODOO));

        // Get Configuration Data
        HealingWaveDistance=GetConfigFloat("distance", HealingWaveDistance, thisRaceID, SKILL_HEALINGWAVE);
        GetConfigFloatArray("amount",  HealingWaveAmountArr, sizeof(HealingWaveAmountArr),
                HealingWaveAmountArr, thisRaceID, SKILL_HEALINGWAVE);

        GetConfigFloatArray("chance",  HexChanceArr, sizeof(HexChanceArr),
                HexChanceArr, thisRaceID, SKILL_HEX);
#endif
...
#if defined SOURCECRAFT
                    if (value)
                        value = CanProcessUpgrade(i,thisRaceID,SKILL_HEX);
#endif
