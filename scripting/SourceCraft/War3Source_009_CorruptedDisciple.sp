...
public void OnWar3LoadRaceOrItemOrdered(num)
{
    ...
    {
#if defined SOURCECRAFT
        thisRaceID=CreateRace("cd", .faction=Hellbourne, .type=Undead, .required_level=32);
#else
        ...
        SetUpgradeCost(thisRaceID, SKILL_STATIC, 10);
        SetUpgradeEnergy(thisRaceID, SKILL_STATIC, 1.0);

        AbilityCooldownTime = GetUpgradeCooldown(thisRaceID,SKILL_TIDE);
        SetUpgradeEnergy(thisRaceID, SKILL_TIDE, AbilityCooldownTime);
        SetUpgradeCooldown(thisRaceID, SKILL_TIDE, AbilityCooldownTime);
        ...
        SetUpgradeCost(thisRaceID, ULT_OVERLOAD, 30);
        SetUpgradeCategory(thisRaceID, ULT_OVERLOAD, 1);
        ult_cooldown=GetConfigFloat("cooldown_on_invoke", ult_cooldown, thisRaceID, ULT_OVERLOAD);
        SetUpgradeEnergy(thisRaceID, ULT_OVERLOAD, ult_cooldown);

        // Get Configuration Data
        ElectricTideRadius=GetConfigFloat("radius", ElectricTideRadius, thisRaceID, SKILL_TIDE);

        GetConfigArray("damage",  ElectricTideMaxDamage, sizeof(ElectricTideMaxDamage),
                ElectricTideMaxDamage, thisRaceID, SKILL_TIDE);

        ConduitDuration=GetConfigNum("duration", ConduitDuration, thisRaceID, SKILL_CONDUIT);

        GetConfigArray("damage",  ConduitPerHit, sizeof(ConduitPerHit),
                ConduitPerHit, thisRaceID, SKILL_CONDUIT);

        StaticHealRadius=GetConfigNum("radius", StaticHealRadius, thisRaceID, SKILL_STATIC);

        GetConfigFloatArray("heal_percent",  StaticHealPercent, sizeof(StaticHealPercent),
                StaticHealPercent, thisRaceID, SKILL_STATIC);

        OverloadDuration=GetConfigNum("duration", OverloadDuration, thisRaceID, ULT_OVERLOAD);
        OverloadRadius=GetConfigNum("radius", OverloadRadius, thisRaceID, ULT_OVERLOAD);

        GetConfigArray("damage_per_hit",  OverloadDamagePerHit, sizeof(OverloadDamagePerHit),
                OverloadDamagePerHit, thisRaceID, ULT_OVERLOAD);

        GetConfigFloatArray("damage_increase",  OverloadDamageIncrease, sizeof(OverloadDamageIncrease),
                OverloadDamageIncrease, thisRaceID, ULT_OVERLOAD);
#endif
...
#if defined SOURCECRAFT
                War3_CooldownMGR(client,ult_cooldown,thisRaceID,ULT_OVERLOAD,_,_);
#else
    ...
    if(userid&&attacker_userid&&userid!=attacker_userid)
    {
        ...
        if(victim>0&&attacker>0)
        {
            ...
            if(/*race_attacker==thisRaceID&&*/!W3HasImmunity(victim,Immunity_Skills)){
                ...
                int skill_level =War3_GetSkillLevel(attacker,thisRaceID,SKILL_CONDUIT);
            ...
            if(ConduitSubtractDamage[attacker]){
                if(ValidPlayer(ConduitBy[attacker],false)){
                    ...
                    int heal =ConduitSubtractDamage[attacker]-dmg;
                    ...
                    if(ConduitLastTime[victim]<1.0 || GetGameTime()-ConduitLastTime[victim]>0.25){
            ...
            int skill = War3_GetSkillLevel(victim,thisRaceID,SKILL_STATIC);
            if(skill>0){
                if(!Hexed(victim,false)&&GetRandomFloat(0.0,1.0)<0.5){
#if defined SOURCECRAFT
                    if (CanInvokeUpgrade(victim,thisRaceID,SKILL_STATIC, .notify=false))
                    {
#endif
                    int heal =RoundFloat(StaticHealPercent[skill]*dmg);
...
public void OnClientPutInServer(i){
    ConduitBy[i]=0;
    ConduitUntilTime[i]=0.0;
    ConduitSubtractDamage[i]=0;
}
