...
public void OnWar3LoadRaceOrItemOrdered(num)
{
    if(num==200)
    {
        ...
        thisRaceID=CreateRace("dragonborn_o", .faction=TheLegion, .type=Biological, .required_level=32);
#else
        ...
        SetUpgradeCost(thisRaceID, SKILL_SCALES, 5);
        SetUpgradeCost(thisRaceID, SKILL_DRAGONBORN, 0);

        SetUpgradeCost(thisRaceID, SKILL_ROAR, 30);
        SetUpgradeCategory(thisRaceID, SKILL_ROAR, 2);
        SetUpgradeCooldown(thisRaceID, SKILL_ROAR, RoarCooldownTime);
        RoarCooldownTime = GetUpgradeCooldown(thisRaceID,SKILL_ROAR);
        SetUpgradeEnergy(thisRaceID, SKILL_ROAR, RoarCooldownTime);

        SetUpgradeCost(thisRaceID, ULTIMATE_DRAGONBREATH, 30);
        SetUpgradeCategory(thisRaceID, ULTIMATE_DRAGONBREATH, 1);
        SetUpgradeCooldown(thisRaceID, ULTIMATE_DRAGONBREATH, 25.0);
        SetUpgradeEnergy(thisRaceID, ULTIMATE_DRAGONBREATH, GetUpgradeCooldown(thisRaceID,ULTIMATE_DRAGONBREATH));

        // Get Configuration Data
        RoarRadius=GetConfigFloat("distance", RoarRadius, thisRaceID, SKILL_ROAR);

        GetConfigFloatArray("chance",  RoarDuration, sizeof(RoarDuration),
                RoarDuration, thisRaceID, SKILL_ROAR);

        GetConfigFloatArray("physical_armor",  ScalesPhysical, sizeof(ScalesPhysical),
                ScalesPhysical, thisRaceID, SKILL_SCALES);

        ImmunityChance=GetConfigFloat("distance", ImmunityChance, thisRaceID, SKILL_DRAGONBORN);

        GetConfigFloatArray("range",  DragonBreathRange, sizeof(DragonBreathRange),
                DragonBreathRange, thisRaceID, ULTIMATE_DRAGONBREATH);
#endif
    ...
    if(race==thisRaceID && pressed && userid>1 && IsPlayerAlive(client) )
    {
        ...
        if(ult_level>0)
        {
            if(!Silenced(client)&&War3_SkillNotInCooldown(client,thisRaceID,ULTIMATE_DRAGONBREATH,true))
            {
                ...
                int target = War3_GetTargetInViewCone(client,breathrange,false,23.0,DragonFilter);
...
public InitPassiveSkills(client)
{
    ...
    {
        //dragonborn
        int skilllvl = War3_GetSkillLevel(client,thisRaceID,SKILL_DRAGONBORN);
