#pragma semicolon 1
#pragma newdecls required    ///WE RECOMMEND THE SEMICOLON
...
public void OnWar3LoadRaceOrItemOrdered(num)
{
    ...
    {
#if defined SOURCECRAFT
        thisRaceID=CreateRace("sacredw", .faction=Sentinel, .type=Biological, .required_level=16);
#else
        ...
        SetUpgradeCost(thisRaceID, SKILL_VITALITY, 10);

        SetUpgradeCost(thisRaceID, SKILL_SPEAR, 20);
        SetUpgradeCategory(thisRaceID, SKILL_SPEAR, 2);
        SetUpgradeEnergy(thisRaceID, SKILL_SPEAR, 10.0);
        SetUpgradeRecurringEnergy(thisRaceID, SKILL_SPEAR, 1.0);

        SetUpgradeCost(thisRaceID, SKILL_BLOOD, 10);
        SetUpgradeRecurringEnergy(thisRaceID, SKILL_BLOOD, 1.0);

        SetUpgradeCost(thisRaceID, ULT_BREAK, 30);
        SetUpgradeCategory(thisRaceID, ULT_BREAK, 1);
        SetUpgradeCooldown(thisRaceID, ULT_BREAK, 20.0);
        SetUpgradeEnergy(thisRaceID, ULT_BREAK, GetUpgradeCooldown(thisRaceID,ULT_BREAK));

        // Get Configuration Data
        GetConfigFloatArray("health",  VitalityHealed, sizeof(VitalityHealed),
                VitalityHealed, thisRaceID, SKILL_VITALITY);

        MaxSpearStacks=GetConfigNum("max_stacks", MaxSpearStacks, thisRaceID, SKILL_SPEAR);

        GetConfigArray("damage",  SpearDamage, sizeof(SpearDamage),
                SpearDamage, thisRaceID, SKILL_SPEAR);

        GetConfigFloatArray("attack_speed",  BerserkerBuffASPD, sizeof(BerserkerBuffASPD),
                BerserkerBuffASPD, thisRaceID, SKILL_BLOOD);

        GetConfigFloatArray("damage",  LifeBreakHPVictim, sizeof(LifeBreakHPVictim),
                LifeBreakHPVictim, thisRaceID, ULT_BREAK);

        GetConfigFloatArray("health_used",  LifeBreakHPCaster, sizeof(LifeBreakHPCaster),
                LifeBreakHPCaster, thisRaceID, ULT_BREAK);
#endif
...
public Action Heal_BurningSpearTimer(Handle:h,any:data) //1 sec
{
    ...
    //int SelfDamage;
    int skill;
    for(int i =1;i<=MaxClients;i++) // Iterate over all clients
    {
        ...
        //    }
            ...
            if(VictimSpearTicks[i] >0)
            {
                ...
                {
#if defined SOURCECRAFT
                    if (CanProcessUpgrade(attacker,thisRaceID,SKILL_SPEAR))
                    {
#endif
        ...
        for(int i =1;i<=MaxClients;i++)
        {
            if(ValidPlayer(i,true))
            {
                if(War3_GetRace(i)==thisRaceID)
                {
                    ...
                    int skilllvl = War3_GetSkillLevel(client,thisRaceID,SKILL_BLOOD);
...
#if defined SOURCECRAFT
                        if (CanProcessUpgrade(client,thisRaceID,SKILL_BLOOD))
                        {
#endif
                        int missing =MaxHP-VictimCurHP;
...
public void OnW3TakeDmgBullet(victim,attacker,Float:damage){
    if(ValidPlayer(victim,true)&&ValidPlayer(attacker,false)&&GetClientTeam(victim)!=GetClientTeam(attacker))
    {
        ...
        {
            // Apply Blood buff
            int skilllvl = War3_GetSkillLevel(attacker,thisRaceID,SKILL_SPEAR);
...
{
    int skill = War3_GetSkillLevel(client, thisRaceID, SKILL_SPEAR);
    if(skill>0 && War3_GetRace(client)==thisRaceID && ability==0 && pressed && IsPlayerAlive(client)&&!Silenced(client))
    {
#if defined SOURCECRAFT
        if (!bSpearActivated[client] && CanInvokeUpgrade(client,thisRaceID,SKILL_SPEAR))
#else
        if(!bSpearActivated[client])
#endif
        {
...
public void OnUltimateCommand(client,race,bool:pressed)
{
    ...
    if(race==thisRaceID && pressed && ValidPlayer(client,true) &&!Silenced(client) )
    {
        ...
        if(ult_level>0)
        {
            ...
            int AttackerCurHP = GetClientHealth(client);
            int SelfDamage = RoundToCeil(AttackerMaxHP * LifeBreakHPCaster[ult_level]);
            ...
            if(!Silenced(client)&&War3_SkillNotInCooldown(client,thisRaceID,ULT_BREAK,true))
            {
                ...
                else
                {
                    ...
                    int target = War3_GetTargetInViewCone(client,ultmaxdistance,false,23.0,ConeTargetFilter);
                    if(target>0)
                    {
                        
                        float VictimMaxHP = float(War3_GetMaxHP(target));
                        int Damage = RoundToFloor(LifeBreakHPVictim[ult_level] * VictimMaxHP);
