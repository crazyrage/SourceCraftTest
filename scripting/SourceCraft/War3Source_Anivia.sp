...
#define TE_SendToTeam       TE_SendEffectToTeam
...
int PlasmaBeam,Glow,LaserSprite,BeamSprite,HaloSprite;
Handle hUltimateEndTimer[MAXPLAYERS]=INVALID_HANDLE;

#if !defined SOURCECRAFT
int iManaValue[MAXPLAYERS]=0;
#endif

int iGlacialEntity[MAXPLAYERS]=-1;
int iFlashFrostEnt[MAXPLAYERS]=-1;
...
int BiteDamageArray[5]={0,2,3,4,5};
...
int GlacialDamagePerTick[5] = {0,2,4,6,6};
...
//char explode[] = "ambient/explosions/explode_2.wav";
char flash1[] = "physics/glass/glass_largesheet_break1.wav";
char flash2[] = "physics/glass/glass_largesheet_break2.wav";
char flash3[] = "physics/glass/glass_largesheet_break3.wav";
char crystallize[] = "physics/concrete/boulder_impact_hard1.wav";//"ui/freeze_cam.wav";
char glacial[] = "ambient/levels/canals/windmill_wind_loop1.wav";
...
public OnWar3PluginReady(){
    ...
    */
#if defined SOURCECRAFT
    thisRaceID=CreateRace("anivia", .faction=Sentinel, .type=Elemental, .required_level=64);
#else
    ...
    SetUpgradeCost(thisRaceID, SKILL_1, 30);
    SetUpgradeCategory(thisRaceID, SKILL_1, 2);
    SetUpgradeCooldown(thisRaceID, SKILL_1, 8.0);
    SetUpgradeEnergy(thisRaceID, SKILL_1, GetUpgradeCooldown(thisRaceID,SKILL_1));

    SetUpgradeCost(thisRaceID, SKILL_2, 30);
    SetUpgradeCategory(thisRaceID, SKILL_2, 3);
    SetUpgradeCooldown(thisRaceID, SKILL_2, 8.0);
    SetUpgradeEnergy(thisRaceID, SKILL_2, GetUpgradeCooldown(thisRaceID,SKILL_2));

    SetUpgradeCost(thisRaceID, ULT, 30);
    SetUpgradeCategory(thisRaceID, ULT, 1);
    SetUpgradeCooldown(thisRaceID, ULT, 6.0);
    SetUpgradeEnergy(thisRaceID, ULT, 10.0);

    // Get Configuration Data
    GetConfigArray("damage",  FlashFrostDmg, sizeof(FlashFrostDmg),
            FlashFrostDmg, thisRaceID, SKILL_1);

    GetConfigFloatArray("lifetime",  FlashFrostLifetime, sizeof(FlashFrostLifetime),
            FlashFrostLifetime, thisRaceID, SKILL_1);

    GetConfigFloatArray("duration",  CondenseDuration, sizeof(CondenseDuration),
            CondenseDuration, thisRaceID, SKILL_2);

    GetConfigFloatArray("chance",  BiteChanceArr, sizeof(BiteChanceArr),
            BiteChanceArr, thisRaceID, SKILL_3);

    GetConfigArray("damage",  BiteDamageArray, sizeof(BiteDamageArray),
            BiteDamageArray, thisRaceID, SKILL_3);

    GetConfigFloatArray("distance",  GlacialMaxDistance, sizeof(GlacialMaxDistance),
            GlacialMaxDistance, thisRaceID, ULT);

    GetConfigFloatArray("radius",  GlacialMaxRadius, sizeof(GlacialMaxRadius),
            GlacialMaxRadius, thisRaceID, ULT);

    GetConfigFloatArray("time",  GlacialMaxTime, sizeof(GlacialMaxTime),
            GlacialMaxTime, thisRaceID, ULT);

    GetConfigFloatArray("slowdown",  GlacialSlowdown, sizeof(GlacialSlowdown),
            GlacialSlowdown, thisRaceID, ULT);

    GetConfigArray("damage",  GlacialDamagePerTick, sizeof(GlacialDamagePerTick),
            GlacialDamagePerTick, thisRaceID, ULT);
#endif
...
public OnRaceChanged(client,oldrace,newrace) {
    ...
    if(bExtension) {
        if(newrace==thisRaceID) {
            W3SetMana(client,bActive,true,thisRaceID);
        }
        else if(oldrace==thisRaceID) {
            W3SetMana(client,bActive,false,thisRaceID);
        }
    }
#endif
    ResetArrayVals(client);
    ...
    if (ref>0) {
        ...
        if(old_mana>=10) {
...
#if defined SOURCECRAFT
            ScaleVector(fAngleVec, shardSpeed);
#else
            ...
            if(entity > 0 && IsValidEntity(entity)) {
                ...
                CreateTimer(0.1, ShardThink, entity);
                CreateTimer(duration,Timer_ShardOvertime,EntIndexToEntRef(entity));
                ...
                iFlashFrostEnt[client]=EntIndexToEntRef(entity);
...
stock W3_Condense(client,skill,team){
    ...
    if (beam_ent > 0 && IsValidEdict(beam_ent))
    {
        ...
        CreateTimer(duration, Timer_RemoveEntity, EntIndexToEntRef(beam_ent));
    }
}
...
stock SpawnFrozenProp(const Float:Origin[3],const Float:fAngles[3],String:modelName[],iHealth,iTeamNum,Float:fLifetime,bool:bDrawInvisible=false) {
    ...
    SetEntProp(PhysicsProp, Prop_Send, "m_CollisionGroup", 6);
    SetEntProp(PhysicsProp, Prop_Send, "m_usSolidFlags", 5);
    SetEntProp(PhysicsProp, Prop_Send, "m_nSolidType", 6);
    ...
    if(iHealth>0) {
        SetEntityHealth(PhysicsProp, iHealth);
                ...
                if(GetVectorDistance(clientVec,aimVec) <= max_distance)
                {
                    if(!bUltActive[client]) {
                        ...
                        */
                        ...
                        W3Hint(client,HINT_SKILL_STATUS,2.0,"Glacial Storm cast!");
                    }
                    else {
                        bUltActive[client]=false;
                        decl Handle:pack;
...
public Action Timer_GlacialStormFX(Handle:timer, Handle:pack) //some point of lazy-ness :S
{
    ...
    if(bUltActive[client]==true&&ValidPlayer(client,false)) {
        ...
        TE_SetupBubbles(aimVec,aimVec,Glow,radius,12,GetRandomFloat(28.0,150.0));
#if defined SOURCECRAFT
        TE_SendEffectToAll();
#else
        TE_SendToAll();
...
public Action Timer_UltimateOvertimed(Handle:timer, Handle:pack)
{
    ...
    if(entity>0) {
        ...
        if(IsValidEntity(entity)) {
            ...
            CreateTimer(1.0, Timer_RemoveEntity, EntIndexToEntRef(entity));
        }
    }
}
...
GetManaWrapper(client) {
    ...
    if(bExtension) {
#endif
        mana = W3GetMana(client,ManaType:iValue);
#if !defined SOURCECRAFT
    }
    else {
...
SetManaWrapper(client,value) {
    ...
    if(bExtension) {
#endif
        W3SetMana(client,ManaType:iValue,value);
#if !defined SOURCECRAFT
    }
    else {
