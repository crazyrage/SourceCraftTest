...
int LucentBeamMin[5] = {0, 3, 4, 5, 6};
int LucentBeamMax[5] = {0, 7, 8, 9, 10};

float GlaiveRadius[5] = {0.0,250.0,300.0,350.0,400.0};
float GlaiveChance = 0.22;
int GlaiveDamage[5] = {0,4,6,8,12};

float BlessingRadius = 280.0;
int BlessingIncrease[5] = {0,1,2,2,3};

float EclipseRadius=500.0;
int EclipseAmount[5]= {0,4,6,8,10};

int SKILL_MOONBEAM,SKILL_BOUNCE,SKILL_AURA,ULT;
int LightModel;
int XBeamSprite,CoreSprite,MoonSprite,HaloSprite;
//new BlueSprite;
...
public OnWar3LoadRaceOrItemOrdered(num)
{
    ...
    {
#if defined SOURCECRAFT
        thisRaceID=CreateRace("luna", .faction=Sentinel, .type=Biological, .required_level=32);
#else
        ...
        SetUpgradeCost(thisRaceID, SKILL_AURA, 20);

        SetUpgradeCost(thisRaceID, SKILL_BOUNCE, 20);
        SetUpgradeEnergy(thisRaceID, SKILL_BOUNCE, 1.0);

        SetUpgradeCost(thisRaceID, SKILL_MOONBEAM, 20);
        SetUpgradeCooldown(thisRaceID, SKILL_MOONBEAM, 3.0);
        SetUpgradeEnergy(thisRaceID, SKILL_MOONBEAM, GetUpgradeCooldown(thisRaceID,SKILL_MOONBEAM));

        SetUpgradeCost(thisRaceID, ULT, 30);
        SetUpgradeCategory(thisRaceID, ULT, 1);
        SetUpgradeCooldown(thisRaceID, ULT, 25.0);
        SetUpgradeEnergy(thisRaceID, ULT, GetUpgradeCooldown(thisRaceID,ULT));

        // Get Configuration Data
        GetConfigArray("min",  LucentBeamMin, sizeof(LucentBeamMin),
                LucentBeamMin, thisRaceID, SKILL_MOONBEAM);

        GetConfigArray("max",  LucentBeamMax, sizeof(LucentBeamMax),
                LucentBeamMax, thisRaceID, SKILL_MOONBEAM);

        GlaiveChance=GetConfigFloat("chance", GlaiveChance, thisRaceID, SKILL_BOUNCE);

        GetConfigArray("radius",  GlaiveRadius, sizeof(GlaiveRadius),
                GlaiveRadius, thisRaceID, SKILL_BOUNCE);

        BlessingRadius=GetConfigFloat("radius", BlessingRadius, thisRaceID, SKILL_AURA);

        GetConfigArray("increase",  BlessingIncrease, sizeof(BlessingIncrease),
                BlessingIncrease, thisRaceID, SKILL_AURA);

        EclipseRadius=GetConfigFloat("radius", EclipseRadius, thisRaceID, ULT);

        GetConfigArray("amount",  EclipseAmount, sizeof(EclipseAmount),
                EclipseAmount, thisRaceID, ULT);
#endif
...
int EclipseOwner[MAXPLAYERSCUSTOM];
int EclipseAmountLeft[MAXPLAYERSCUSTOM];
