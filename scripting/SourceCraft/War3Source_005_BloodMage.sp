...
public void OnMapStart()
{
    ...
    for(int i =1;i<=MAXPLAYERSCUSTOM;i++)
    {
...
public void OnUltimateCommand(client,race,bool:pressed)
{
    ...
    if(race==thisRaceID && pressed && userid>1 && IsPlayerAlive(client) )
    {
        ...
        {
#if !defined SOURCECRAFT
            int max_ult =GetConVarInt(UltimateMaxCvar);
#endif
            ...
            {
                /////Flame Strike
                int target = War3_GetTargetInViewCone(client,UltimateMaxDistance[ult_level],false,23.0,IsBurningFilter);
                if(target>0)
                {
                    ++UltimateUsed[client];
                    ...
                    if(War3_GetGame()==Game_CS) {
                        ...
                        effect_vec[2]-180;
                        int ent = AttachParticle(target, "env_fire_medium_smoke", effect_vec, "rfoot");
...
public Action BurnLoop(Handle:timer,any:userid)
{
    ...
    if(victim>0 && attacker>0 && BurnsRemaining[victim]>0 && IsClientInGame(victim) && IsClientInGame(attacker) && IsPlayerAlive(victim))
    {
        ...
        int damage = War3_GetGame()==Game_TF? ULT_DAMAGE_TF:ULT_DAMAGE_CS;
        ...
        if(War3_GetRace(attacker)==thisRaceID)
        {
            ...
            if(IsPlayerAlive(attacker)&&IsPlayerAlive(victim))
            {
                if(!W3HasImmunity(victim,Immunity_Skills))
                {
                    ...
                    int skill_level =War3_GetSkillLevel(attacker,thisRaceID,SKILL_BANISH);
                    if(skill_level>0)
                    {
...
#endif
                                {
                                    int stolen =RoundToCeil(float(GetMoney(victim))*MoneyStealPercent[attacker]);
                                    int new_money =GetMoney(attacker)+stolen;
...
#endif
                                {
                                    int stolen =RoundFloat(float(War3_GetCurrency(victim))*TFCreditStealPercent);
