...
char HurtSound[][]  = { "player/pl_pain5.wav",  "player/pl_pain6.wav",
                                    "player/pl_pain7.wav", "player/pain.wav" };

char DeathSound[][]  = { "player/death.wav" };

int ClientInfected[MAXPLAYERS + 1];
int ClientInfectionSource[MAXPLAYERS + 1];
int InfectionFlags:ClientInfectionFlags[MAXPLAYERS + 1];
...
Handle Cvar_DmgAmount = INVALID_HANDLE;
Handle Cvar_DmgTime = INVALID_HANDLE;

Handle Cvar_InfectEnable = INVALID_HANDLE;
Handle Cvar_InfectDistance = INVALID_HANDLE;
Handle Cvar_InfectDuration = INVALID_HANDLE;
Handle Cvar_InfectContagious = INVALID_HANDLE;

Handle Cvar_InfectMedi = INVALID_HANDLE;
Handle Cvar_InfectMediCheckTime = INVALID_HANDLE;
Handle Cvar_InfectSyringe = INVALID_HANDLE;

Handle Cvar_InfectMedics = INVALID_HANDLE;
Handle Cvar_InfectHeal = INVALID_HANDLE;
Handle Cvar_InfectSameTeam = INVALID_HANDLE;
Handle Cvar_InfectOpposingTeam = INVALID_HANDLE;

Handle Cvar_InfectFailedDelay = INVALID_HANDLE;
Handle Cvar_InfectSucceededDelay = INVALID_HANDLE;

Handle Cvar_SpreadEnable = INVALID_HANDLE;
Handle Cvar_SpreadDistance = INVALID_HANDLE;
Handle Cvar_SpreadCheckTime = INVALID_HANDLE;

Handle Cvar_SpreadAll = INVALID_HANDLE;
Handle Cvar_SpreadInfector = INVALID_HANDLE;
Handle Cvar_SpreadMedics = INVALID_HANDLE;
Handle Cvar_SpreadSameTeam = INVALID_HANDLE;
Handle Cvar_SpreadOpposingTeam = INVALID_HANDLE;

Handle Cvar_SameColors = INVALID_HANDLE;
Handle Cvar_GunColors = INVALID_HANDLE;
Handle Cvar_BothTeamsRed, Handle:Cvar_BothTeamsBlue, Handle:Cvar_BothTeamsGreen, Handle:Cvar_BothTeamsAlpha;
Handle Cvar_RTeamRed, Handle:Cvar_RTeamBlue, Handle:Cvar_RTeamGreen, Handle:Cvar_RTeamAlpha;
Handle Cvar_BTeamRed, Handle:Cvar_BTeamBlue, Handle:Cvar_BTeamGreen, Handle:Cvar_BTeamAlpha;

Handle InfectionTimer = INVALID_HANDLE;
Handle MediTimer = INVALID_HANDLE;
Handle SpreadTimer = INVALID_HANDLE;

Handle OnInfectedHandle = INVALID_HANDLE;
Handle OnInfectionHurtHandle = INVALID_HANDLE;

Handle g_precacheTrie = INVALID_HANDLE;
...
public OnPluginStart()
{
    if (GetGameType() == tf2)
    {
        ...
        CvarAnnounce = CreateConVar("medic_infect_announce", "0", "This will enable announcements that the plugin is loaded", FCVAR_NONE);

        Cvar_InfectMedi = CreateConVar("sv_medic_infect_medi", "1", "Infect using medi gun", FCVAR_NONE);
        Cvar_InfectMediCheckTime = CreateConVar("sv_medic_infect_medi_check_time", "0.2", "Amount of time between checks, 0.0 for gameframe", FCVAR_NONE, true, 0.0);
        Cvar_InfectSyringe = CreateConVar("sv_medic_infect_syringe", "1", "Infect using syringe gun", FCVAR_NONE);

        Cvar_InfectHeal = CreateConVar("sv_medic_infect_heal", "1", "Allow medics to uninfect players", FCVAR_NONE);

        Cvar_InfectMedics = CreateConVar("sv_medic_infect_medics", "1", "Allow medics to be infected", FCVAR_NONE);
        Cvar_SpreadMedics = CreateConVar("sv_medic_infect_spread_medics", "1", "Should medics be vaccinated?", FCVAR_NONE);
    }
    ...
    CvarEnable = CreateConVar("medic_infect_on", "1", "1 turns the plugin on 0 is off", FCVAR_NONE);

    Cvar_DmgAmount = CreateConVar("sv_medic_infect_dmg_amount", "2", "Amount of damage medic infect does each heartbeat",FCVAR_NONE);
    Cvar_DmgTime = CreateConVar("sv_medic_infect_dmg_time", "2.0", "Amount of time between infection heartbeats",FCVAR_NONE, true, 0.0);

    Cvar_InfectEnable = CreateConVar("sv_medic_infect_allow_infect", "1", "Can medics infect?", FCVAR_NONE);
    Cvar_InfectDistance = CreateConVar("sv_medic_infect_infect_distance", "0.0", "Distance infection can be injected from", FCVAR_NONE, true, 0.0);
    Cvar_InfectDuration = CreateConVar("sv_medic_infect_duration", "60.0", "Duration of infections (0.0=unlimited)",FCVAR_NONE, true, 0.0);
    Cvar_InfectContagious = CreateConVar("sv_medic_infect_contagious", "20.0", "How long infection remains contagious (0.0=unlimited)",FCVAR_NONE, true, 0.0);

    Cvar_InfectFailedDelay = CreateConVar("sv_medic_failed_infect_delay", "1.0", "Delay between failed infections", FCVAR_NONE, true, 0.0);
    Cvar_InfectSucceededDelay = CreateConVar("sv_medic_succeeded_infect_delay", "5.0", "Delay between succeeded infections", FCVAR_NONE, true, 0.0);

    Cvar_InfectSameTeam = CreateConVar("sv_medic_infect_friendly", "1", "Allow medics to infect friends", FCVAR_NONE);
    ...
    Cvar_SpreadInfector = CreateConVar("sv_medic_infect_spread_infector", "0", "Should infectors be vaccinated?", FCVAR_NONE);
    Cvar_SpreadSameTeam = CreateConVar("sv_medic_infect_spread_friendly", "1", "Allow medical infections to run rampant inside a team",FCVAR_NONE);
    Cvar_SpreadOpposingTeam = CreateConVar("sv_medic_infect_spread_enemy", "0", "Allow medical infections to run rampant between teams",FCVAR_NONE);

    Cvar_SameColors = CreateConVar("medic_infect_same_colors", "0", "Infected from both teams use same colors", FCVAR_NONE);
    Cvar_GunColors = CreateConVar("medic_infect_gun_colors", "1", "Infected players guns reflect teams", FCVAR_NONE);

    Cvar_BothTeamsRed = CreateConVar("medic_infect_teams_red", "0", "[Both Teams Infected] Amount of Red", FCVAR_NONE, true, 0.0, true, 255.0);
    Cvar_BothTeamsGreen = CreateConVar("medic_infect_teams_green", "255", "[Both Team Infected] Amount of Green", FCVAR_NONE, true, 0.0, true, 255.0);
    Cvar_BothTeamsBlue = CreateConVar("medic_infect_teams_blue", "100", "[Both Team Infected] Amount of Blue", FCVAR_NONE, true, 0.0, true, 255.0);
    Cvar_BothTeamsAlpha = CreateConVar("medic_infect_teams_alpha", "255", "[Both Team Infected] Amount of Transperency", FCVAR_NONE, true, 0.0, true, 255.0);

    Cvar_RTeamRed = CreateConVar("medic_infect_red_team_red", "255", "Amount of Red for the Red Team", FCVAR_NONE, true, 0.0, true, 255.0);
    Cvar_RTeamGreen = CreateConVar("medic_infect_red_team_green", "100", "Amount of Green for the Red Team", FCVAR_NONE, true, 0.0, true, 255.0);
    Cvar_RTeamBlue = CreateConVar("medic_infect_red_team_blue", "60", "Amount of Blue for the Red Team", FCVAR_NONE, true, 0.0, true, 255.0);
    Cvar_RTeamAlpha = CreateConVar("medic_infect_red_team_alpha", "255", "Amount of Transperency for the Red Team", FCVAR_NONE, true, 0.0, true, 255.0);

    Cvar_BTeamRed = CreateConVar("medic_infect_blue_team_red", "0", "Amount of Red for the Blue Team", FCVAR_NONE, true, 0.0, true, 255.0);
    Cvar_BTeamGreen = CreateConVar("medic_infect_blue_team_green", "255", "Amount of Green for the Blue Team", FCVAR_NONE, true, 0.0, true, 255.0);
    Cvar_BTeamBlue = CreateConVar("medic_infect_blue_team_blue", "100", "Amount of Blue for the Blue Team", FCVAR_NONE, true, 0.0, true, 255.0);
    Cvar_BTeamAlpha = CreateConVar("medic_infect_blue_team_alpha", "255", "Amount of Transperency for the Blue Team", FCVAR_NONE, true, 0.0, true, 255.0);
}
...
public OnConfigsExecuted()
{
    ...
    if (GetGameType() == tf2)
    {
        timeval = GetConVarFloat(Cvar_InfectMediCheckTime);
        if (timeval > 0.0)
...
public Action HandleInfection(Handle:timer)
{
    ...
    float InfectDuration = GetConVarFloat(Cvar_InfectDuration);
    float InfectContagious = GetConVarFloat(Cvar_InfectContagious);
    for(new client = 1; client <= MaxClients; client++)
    {
        ...
        float InfectionTime = GetEngineTime() - ClientInfectionTime[client];
        ...
        // Remove contagious flag after the time expires
        if (InfectContagious <= 0.0 || InfectContagious >= InfectionTime)
        ...
        else
        {
            ...
            char agent[16];
            if (ClientInfectionFlags[client] & Infection_Irradiation)
                strcopy(agent, sizeof(agent), "radiation");
            else
                strcopy(agent, sizeof(agent), "infection");
...
public bool OnClientConnect(client, String:rejectmsg[], maxlen)
{
    ...
    ClientInfectionFlags[client] = Infection_Normal;
...
public Action MedicModify(Handle:event, const char name[], bool:dontBroadcast)
{
    // Skip feigned deaths.
    ...
    ClientInfectionFlags[id] = Infection_Normal;
...
public MedicInfect(to, from, bool:friendlyInfect, bool:irradiate)
{
    ...
    else
    {
        ...
        Infect(to, from, friendlyInfect, true, irradiate);
    }
}
...
public Action SpreadInfection(Handle:timer)
{
    ...
    for (client = 1; client <= MaxClients; client++)
    {
        ...
        {
            if (ClientInfectionFlags[client] & Infection_Contagious)
            {
...
stock TransmitInfection(to,from)
{
    ...
    else if (Cvar_SpreadMedics && !GetConVarInt(Cvar_SpreadMedics) &&
             (TF2_GetPlayerClass(to) == TFClass_Medic)  )
    {
        return;
    }
...
stock Infect(to, from, bool:friendly, bool:infect, bool:irradiate)
{
    if(!NativeControl && !GetConVarBool(CvarEnable))
        return; 
    else if (ClientInfected[to])
    {
        ...
        ClientInfectionFlags[to] |= Infection_Contagious;
        return;
    }
    ...
    if (NativeHooked)
    {
        ...
        Call_PushCell(to);
        Call_PushCell(from);
        Call_PushCell(ClientInfectionSource[from]);
        Call_PushCell(true);
    ...
    if (friendly)
        ClientInfectionFlags[to] |= Infection_Friendly;

    if (irradiate)
        ClientInfectionFlags[to] |= Infection_Irradiation;

    char infected[16], String:agent[16];
    if (ClientInfectionFlags[to] & Infection_Irradiation)
    {
        strcopy(agent, sizeof(agent), "Radiation");
        strcopy(infected, sizeof(infected), "irradiated");
    }
    else
    {
        strcopy(agent, sizeof(agent), "Virus");
        strcopy(infected, sizeof(infected), "infected");
    }

    PrintHintText(to, "You have been %s!", infected);

    if (from)
    {
        LogToGame("%L %s %L", from, infected, to);
        PrintToConsole(from, "%L %s %L", from, infected, to);
        PrintToConsole(to, "%L %s %L", from, infected, to);
        if (infect)
            PrintHintText(from, "%s administered!", agent);
        else
            PrintHintText(from, "%s spread!", agent);
    }
    else
    {
        LogToGame("%L was %s", to, infected);
        PrintToConsole(to, "%L was %s", to, infected);
    }
}
...
stock UnInfect(to, from=0)
{
    ...
    if (NativeHooked)
    {
        ...
        Call_PushCell(to);
        Call_PushCell(from);
        Call_PushCell(ClientInfectionSource[to]);
        Call_PushCell(false);
        Call_PushArray(color, sizeof(color));
        Call_Finish(result);
        if (result != Plugin_Continue)
    ...
    if (IsPlayerAlive(to))
    {
        ...
        {
            if (ClientInfectionFlags[to] & Infection_Irradiation)
                PrintHintText(to, "The radiation has run it's course!");
            else
        ...
        {
            if (ClientInfectionFlags[to] & Infection_Irradiation)
                PrintHintText(to, "You have been decontaminated!");
            else
    ...
    {
        if (ClientInfectionFlags[to] & Infection_Irradiation)
            PrintHintText(from, "Anti-Radiation administered!");
        else
            PrintHintText(from, "Anti-Virus administered!");
    }

    ClientInfected[to] = 0;
    ClientInfectionFlags[to] = Infection_Normal;
...
stock PrepareSound(const char sound[], bool:preload=false)
{
...
{
    if (!NativeControl && GetGameType() == tf2 &&
        GetConVarBool(CvarEnable) && GetConVarBool(CvarAnnounce))
    {
        ...
        bool spreadAll = GetConVarBool(Cvar_SpreadAll);
...
public PlayerSpawnEvent(Handle:event,const char name[],bool:dontBroadcast)
{
    ...
    ClientInfectionFlags[index] = Infection_Normal;
}
    ...
    if (victim > 0)
    {
            ...
            IsClientInGame(attacker) && IsPlayerAlive(attacker)) 
        {
            if (NativeControl)
            {
                ...
                {
                    MedicInfect(victim, attacker, false, false);
        ...
        if (NativeControl && GetGameType() == tf2 && !infected)
        {
                ...
                IsClientInGame(assister) && IsPlayerAlive(assister)) 
            {
                ...
                {
                    MedicInfect(victim, assister, false, false);
...
public Native_HookInfection(Handle:plugin,numParams)
{
    if(numParams >= 1)
    {
        AddToForward(OnInfectedHandle, plugin, GetNativeCell(1));
        NativeHooked = true;
    }
}
...
public Native_HookInfectionHurt(Handle:plugin,numParams)
{
    if(numParams >= 1)
    {
        AddToForward(OnInfectionHurtHandle, plugin, GetNativeCell(1));
...
public Native_MedicInfect(Handle:plugin,numParams)
{
    if (numParams >= 1)
    {
        ...
        if (client > 0)
        {
            MedicInfect(target, client, allow, irradiate);
            MedicInfectDelay[client] = GetGameTime() + GetConVarFloat(Cvar_InfectSucceededDelay);
        }
        else
            Infect(target, client, allow, true, irradiate);
    }
}
...
public Native_Infect(Handle:plugin,numParams)
{
    if (numParams >= 1)
    {
        ...
        bool irradiate = (numParams >= 4) ? (bool:GetNativeCell(4)) : false;
        bool allow = (numParams >= 3) ? (bool:GetNativeCell(3)) : false;
        bool friendly = (target > 0 && client > 0 &&
                             IsClientInGame(target) && IsClientInGame(client) &&
                             GetClientTeam(target) == GetClientTeam(client));

        if (allow || !friendly)
            Infect(target, client, friendly, true, irradiate);
    }
}
