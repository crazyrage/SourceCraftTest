...
int TFExtObjectType:g_RemoteType[MAXPLAYERS+1];
...
// forwards
Handle g_fwdOnBuildCommand = INVALID_HANDLE;
Handle fwdOnBuildObject = INVALID_HANDLE;
Handle fwdOnControlObject = INVALID_HANDLE;

// convars
Handle cvarObjectsTxt = INVALID_HANDLE;
Handle cvarRemote = INVALID_HANDLE;
Handle cvarSteal = INVALID_HANDLE;
Handle cvarZombie = INVALID_HANDLE;
Handle cvarBuild = INVALID_HANDLE;
Handle cvarLevel = INVALID_HANDLE;
Handle cvarMini = INVALID_HANDLE;
Handle cvarInstant = INVALID_HANDLE;
Handle cvarAlways = INVALID_HANDLE;
Handle cvarFactor = INVALID_HANDLE;
Handle cvarSpeed = INVALID_HANDLE;
Handle cvarJump = INVALID_HANDLE;
Handle cvarFall = INVALID_HANDLE;

Handle cvarBuildEnabled = INVALID_HANDLE;
Handle cvarBuildImmunity = INVALID_HANDLE;

#if defined _amp_node_included
    Handle cvarAmp = INVALID_HANDLE;
    Handle cvarRepair = INVALID_HANDLE;
#endif
...
int HasBuiltFlags:g_HasBuilt[MAXPLAYERS+1];
...
{		
    CreateConVar("sm_remote_version", PLUGIN_VERSION, "Remote Control/Build/Limit Buildings Version", FCVAR_DONTRECORD|FCVAR_REPLICATED|FCVAR_NOTIFY);
        ...
        if ((permissions & REMOTE_CAN_BUILD) != 0)
        {
            ...
            AddMenuItem(menu,"0","Build a Dispenser",
                ...
                if (m_AmpNodeAvailable)
                {
                    ...
                    if ((permissions & REMOTE_CAN_BUILD_UPGRADE) != 0)
                    {
                        AddMenuItem(menu,"9","Build an Upgrade Station",
                                    (counts[TFExtObject_Dispenser] >= g_isAllowed[client][TFExtObject_Dispenser])
                                    ? ITEMDRAW_DISABLED : ITEMDRAW_DEFAULT);
                    }
                }
            ...
            if ((permissions & REMOTE_CAN_BUILD_LEVEL_1) != 0)
            {
                ...
                {
                    AddMenuItem(menu,"10","Build a Level 1 Sentry Gun", flag);
                    AddMenuItem(menu,"11","Build a Level 2 Sentry Gun", flag);
                    if ((permissions & REMOTE_CAN_BUILD_LEVEL_3) != 0)
                        AddMenuItem(menu,"12","Build a Level 3 Sentry Gun", flag);
                }
            }

            AddMenuItem(menu,"13","Destroy a Structure", ((sum > 0) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED));
        }
...
BuildMenu(client, permissions, bool:control)
{
    ...
    if ((permissions & REMOTE_CAN_BUILD_LEVEL_1) != 0)
    {
        ...
        {
            AddMenuItem(menu,"10","Level 1 Sentry Gun", flag);
            AddMenuItem(menu,"11","Level 2 Sentry Gun", flag);
            if ((permissions & REMOTE_CAN_BUILD_LEVEL_3) != 0)
                AddMenuItem(menu,"12","Level 3 Sentry Gun", flag);
        }
    }

    AddMenuItem(menu,"13","Destroy Structure", ((sum > 0) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED));
    DisplayMenu(menu,client,MENU_TIME_FOREVER);
}
...
public BuildSelected(Handle:menu,MenuAction:action,client,selection)
{
    if (action == MenuAction_Select)
    {
        ...
        else
        {
            ...
            else if (item == 7 || item == 8 || item == 9) // Amplifier || Repair Node || Upgrade Station
            {
                type = TFExtObject_Dispenser;
                level = 1;
            }
                    ...
                    bool:drop=true, bool:check=true, Float:objectPosition[3]={0.0})
{
    ...
    if (res == Plugin_Continue)
    {
        ...
        switch (type)
        {
            ...
            case TFExtObject_Dispenser,  TFExtObject_Amplifier,
                 TFExtObject_RepairNode, TFExtObject_UpgradeStation:
            {
...
CountBuildings(client, counts[TFExtObjectType])
{
    ...
    counts[TFExtObject_UpgradeStation]  = counts[TFExtObject_Dispenser];
...
public Action RemoteOff(client, args)
{
    ...
    g_WatcherEntRef[client] = INVALID_ENT_REFERENCE;
    g_RemoteObjectRef[client] = INVALID_ENT_REFERENCE;
        ...
        if (event != INVALID_HANDLE)
        {
            ...
            if (m_AmpNodeAvailable)
            {
                switch (type)
                {
                    ...
                    case TFExtObject_UpgradeStation:
                    {
                        ConvertToUpgradeStation(iDispenser);
                    }
                }
            }
        #endif
