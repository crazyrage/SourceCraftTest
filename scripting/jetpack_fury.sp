 ...
 * File: thefury.sp
 * Description: Jetpack the Fury for TF2 Pyros.
 * Author(s): RavensBro
 */
...
Handle g_BurnEnemy = INVALID_HANDLE;
Handle g_BurnEnemy2 = INVALID_HANDLE;
Handle g_JetPackUp = INVALID_HANDLE;
Handle g_JetPackUpward = INVALID_HANDLE;
Handle g_JetPackFall = INVALID_HANDLE;
Handle g_JetPackFrwd = INVALID_HANDLE;
Handle cookie_jet;
...
public OnPluginStart()
{
    ...
    g_BurnEnemy2 = CreateConVar("thefury_explode", "1", _, 0, true, 0.0, true, 1.0);
    g_JetPackUp = CreateConVar("thefury_up_mag", "1.57", "Up velocity magnification", 0, true, 0.0, true, 2.0);
    ...
    cookie_jet = RegClientCookie("Jet Pack", "Jetpack on or off", CookieAccess_Private);
...
public Action OnPlayerSay(client, args)
{
    ...
    if(strcmp(text[startidx], "!jetpack", false) == 0)
    {
        showmenu(client);
    }
}
...
loadcookies(client)
{
    ...
    if(!StrEqual(buffer, ""))
    {
        jetpack_pref[client] = StringToInt(buffer);
    }
}
                            ...
                            if(CheckElapsedTime(iClient, 0.1))
                            {
                                if(GetClientButtons(iClient) & IN_JUMP)
                                {
                                    if(!(GetEntityFlags(iClient) & FL_ONGROUND) && !(GetEntityFlags(iClient) & FL_INWATER) &&
                                       !(GetEntityFlags(iClient) & FL_SWIM) && g_ReleaseButton[iClient])
                                    {
                                        if(!g_State[iClient])
                                        {
...
{	
    {
        ...
        ResetPlayerData(iClient);
        CreateTimer(0.001, Timer_HookRagdoll, iClient);
        return Plugin_Continue;
    ...
    if(GetClientTeam(iClient)>1)
    {
        for(new iVictim = 1; iVictim <= MaxClients; iVictim++)
        {
            if( iVictim<=0 || iVictim>MaxClients || !IsClientConnected(iVictim) ||
                ...
                iVictim==iClient || GetClientTeam(iVictim)<=1 )
            {
                continue;
            }
            else if( !bFF && GetClientTeam(iVictim)==GetClientTeam(iClient) )
                continue;
            else
            {
                float PlayerVec[3];
