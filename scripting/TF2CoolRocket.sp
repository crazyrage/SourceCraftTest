 ...
 * File: TF2CoolRocket.sp
 * Description: TF2CoolRocket
 * Author(s): javalia
 * Modified by: -=|JFH|=-Naris (Murray Wilson)
 */

char PLUGIN_VERSION[60] = "1.0.2.3";
...
Handle g_cvarInduceWeaponList = INVALID_HANDLE;
Handle g_cvarArrowToHead = INVALID_HANDLE;
...
public OnEntityCreated(entity, const char classname[])
{
    ...
    if (StrContains(classname, "tf_projectile_", false) == 0)
    {
        ...
        if (StrContains(cvarstring, classname, false) != -1)
        {
            ...
            g_bToHead[entity] = (StrEqual(classname, "tf_projectile_arrow", false) &&
                                 GetConVarBool(g_cvarArrowToHead));
    ...
    else
    {
        ...
        targetvalid = !(!TF2_IsPlayerInCondition(target, TFCond_Disguising)
                        ...
                        && (GetEntProp(target, Prop_Send, "m_nDisguiseTeam")
                            == GetEntProp(entity, Prop_Data, "m_iTeamNum")));
    }
