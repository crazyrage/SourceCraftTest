 ...
 * File: sentryguns.sp
 * Description: Sentry Guns for mods other than TF2
 * Author(s): -=|JFH|=-Naris (Murray Wilson)
 * Ported from: gmsentryguns.sma by JGHG & GlobalModders.net Scripting Team
 */

#endinput // Not completed yet!
...
{
	RegConsoleCmd("sentry_build", createsentryhere, "- build a sentry gun where you are");
	RegConsoleCmd("sentry_menu", menumain, "- displays Sentry gun menu");
	RegConsoleCmd("say", check_say);
	RegConsoleCmd("say_team", check_say);

#if defined DEBUG
	RegConsoleCmd("0botbuild", botbuild_fn, "- force bots to build right where they are (debug)", ADMIN_CFG);
#endif

	//CreateConVar("pend_inc", "30");
	//CreateConVar("radar_increment", "4.56");
	//CreateConVar("spycamoffset", "24.0");

	sentry_max = CreateConVar("sentry_max", "2");
	sentry_cost1 = CreateConVar("sentry_cost1", "1000");
	sentry_cost2 = CreateConVar("sentry_cost2", "500");
	sentry_cost3 = CreateConVar("sentry_cost3", "250");
	sentry_team = CreateConVar("sentry_team", "0");
	sentry_wait = CreateConVar("sentry_wait", "15");

	HookEvent("ResetHUD", newround_event, "b");
	HookEvent("SendAudio", endround_event, "a", "2&%!MRAD_terwin", "2&%!MRAD_ctwin", "2&%!MRAD_rounddraw");
	HookEvent("TextMsg", endround_event, "a", "2&#Game_C", "2&#Game_w");
	HookEvent("TextMsg", endround_event, "a", "2&#Game_will_restart_in");
	//Shaman: For destroying sentries after team change
	HookEvent( "TeamInfo", jointeam_event, "a");
